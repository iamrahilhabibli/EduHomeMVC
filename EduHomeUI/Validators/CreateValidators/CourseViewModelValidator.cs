...
public class CourseViewModelValidator : AbstractValidator<CourseViewModel>
{
    public CourseViewModelValidator()
    {
        ...
        RuleFor(vm => vm.StudentCount)
         .NotEmpty().WithMessage("Student count is required")
         .GreaterThan(0).WithMessage("Student count must be greater than zero")
         .Must(BeNumericInt).WithMessage("Student count must be a numeric value");
    ...
    private bool BeNumericInt(int value)
    {
